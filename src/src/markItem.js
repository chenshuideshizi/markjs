...
export default class MarkItem {
  ...
  renderArea() {
    ...
    this.ctx.beginPath();
    let _pointArr = this.pointArr.concat(
      this.area ? (this.areaToPointPos ? [this.areaToPointPos] : []) : []
    );
   ...
   * @Desc: 绘制线段
   */
  renderLines({ lineWidth, strokeColor, lineJoin }, onlyPath) {
    this.ctx.save();
  ...
  drawPoint(x, y, onlyPath, beginPath, reverse) {
    let { customRenderPoint, showPoint, pointType, pointWidth } = this.opt;
    if (beginPath) {
      this.ctx.beginPath();
    }
    // 用户自定义绘制端点方法
  ...
  createLineSegments(close) {
    ...
    let lineSegments = [];
    for (let i = 0; i < len - 1; i++) {
      lineSegments.push([arr[i], arr[i + 1]]);
    }
    // 包含起点和终点组成的线段
    if (close) {
      lineSegments.push([arr[len - 1], arr[0]]);
    }
    return lineSegments;
  }
